...
def init_state(key, default_value):
    """Initialisiert Session State Variablen sicher."""
    if key not in st.session_state:
        st.session_state[key] = default_value

# --- State Initialisierung ---
# Plant Model Defaults
init_state('m_order', "2nd Order System")
init_state('m_v', 2.0)
init_state('m_t1', 10.0)
init_state('m_t2', 1.0)
init_state('m_t3', 0.5)
init_state('m_delay', 0.0)
init_state('m_noise_amp', 0.0)
init_state('m_integrator', False)

# Tuning Defaults
init_state('t_mode_sel', 3) # Auto
init_state('t_set', 150.0)
init_state('t_hyst', 1.0)
init_state('t_max', 100.0)
init_state('t_min', -100.0)
init_state('t_periods', 4)
init_state('t_min_qual', 95.0)

# PID Defaults & Results (WICHTIG: kp, tn, tv sind die Tuning-Ergebnisse)
init_state('kp', 2.0)
init_state('tn', 10.0)
init_state('tv', 1.0)
...
init_state('m_sim_time', 200)
init_state('step_val', 150.0)
...
def sync_tuning_params():
    """
    Kopiert die Ergebnisse des Tunings in die PID-Felder,
    wenn die Checkbox 'Use Tuned Parameters' aktiv ist.
    """
...
def on_upload_callback():
    """Wird ausgeführt, wenn eine Datei hochgeladen wird."""
    if st.session_state.uploader_key is not None:
        try:
            ...
            # Wichtig: Nach dem Laden sicherstellen, dass die Werte synchron sind
            sync_tuning_params()
...
def system_dynamics_generic(x, t, u, gain, time_constants, has_integrator):
    ...
    if has_integrator:
        ...
        current_input = x[state_idx]; state_idx += 1
    else:
...
def simulate_pid_response(time_vector, model_params, pid_conf):
    ...
    pv_output = np.zeros(ns + 1); curr_state = np.zeros(num_states)
    
    delay_samples = int(max(1, np.ceil(model_params['delay'] / dt))) 
    ring_buffer = np.zeros(delay_samples); buffer_idx = 0
    ...
    for i in range(ns):
        ...
        # Anti-Windup
        if pid_conf.anti_windup == "ON" and pid_conf.ctrl_type in ["PI", "PID"]:
...
def main():
    ...
    with tab1:
        with st.sidebar:
            ...
            m_order = st.selectbox("System Structure", ["1st Order System", "2nd Order System", "3rd Order System"], key="m_order")
            m_v = st.slider("Gain (K)", 0.1, 10.0, step=0.1, key="m_v")
            m_integrator = False
            
            time_consts = []
            m_t1 = st.slider("Time Constant 1 (T1)", 0.1, 20.0, step=0.1, key="m_t1")
            time_consts.append(m_t1)
            
            if m_order == "1st Order System":
                m_integrator = st.checkbox("Set Integrator", key="m_integrator")
            
            if m_order in ["2nd Order System", "3rd Order System"]:
                m_t2 = st.slider("Time Constant 2 (T2)", 0.1, 20.0, step=0.1, key="m_t2")
                time_consts.append(m_t2)
                
            if m_order == "3rd Order System":
                m_t3 = st.slider("Time Constant 3 (T3)", 0.1, 20.0, step=0.1, key="m_t3")
                time_consts.append(m_t3)
            
            m_delay = st.slider("Dead Time [s]", 0.0, 10.0, key="m_delay")
            st.divider()
            m_noise_amp = st.slider("Measurement Noise (Amp)", 0.0, 1.0, step=0.01, key="m_noise_amp")
        ...
        for _ in t_open:
            u_del = rb_o[idx_o]; rb_o[idx_o] = 1.0; idx_o = (idx_o + 1) % nd_o
            step_o = odeint(system_dynamics_generic, curr_state, [0, dt_o], args=(u_del, m_v, time_consts, m_integrator))
            noise = np.random.normal(0, m_noise_amp) if m_noise_amp > 0 else 0.0
            pv_o.append(step_o[-1, -1] + noise); curr_state = step_o[-1]
    ...
    with tab2:
        with st.sidebar:
            ...
            if t_mode_sel == 3: 
                t_min_qual = st.slider("Min. Symmetry Quality [%]", 50, 100, step=1, key="t_min_qual")
            else: 
                t_min_qual = 95.0 
            
            st.divider()
            t_set = st.number_input("Setpoint", key="t_set")
            t_hyst = st.number_input("Hysteresis", key="t_hyst")
            t_max = st.number_input("Relay Max", key="t_max")
            t_min = st.number_input("Relay Min", key="t_min")
            t_periods = st.slider("Required Periods", 2, 10, key="t_periods")
        ...
        if st.button("▶️ Start Tuning", type="primary"):
            ...
            for i, _ in enumerate(t_sim):
                ...
                u_delayed = rb_t[idx_t]; rb_t[idx_t] = u_cmd; idx_t = (idx_t + 1) % nd_t
                y_step = odeint(system_dynamics_generic, curr_state, [0, Ts], args=(u_delayed, m_v, model_params['time_constants'], model_params['integrator']))
            ...
            p1_val = tuner.p1_area if tuner.p1_area is not None else 0.0
            p1_str = f"{p1_val:.1f}%" if tuner.p1_area is not None else "N/A"
            c4.metric("Phase 1 Quality", p1_str, delta="Good" if p1_val > 90 else "Low")

            if tuner.current_phase == 2:
                final_qual = tuner.quality_area
                c5.metric("Phase 2 Quality", f"{final_qual:.1f}%", delta="Good" if final_qual > 90 else "Low")
            else:
            ...
            st.success("Tuning fertig! Werte in Tab 3 übernommen.")
    ...
    with tab3:
        with st.sidebar:
            ...
            rk_v_val = st.session_state.rk_v
            rk_tf = rk_v_val / 10.0 if rk_v_val > 0 else 0.0
            
            st.markdown("---")
            m_sim_time = st.slider("Duration [s]", 10, 500, key="m_sim_time")
            step_val = st.number_input("Step Value", key="step_val")

        st.subheader("Closed-Loop Control Performance")
        
        if st.button("🚀 Simulate Step Response", type="primary"):
            p_conf = st.session_state.rk_p
            n_conf = st.session_state.rk_n
            v_conf = st.session_state.rk_v
            type_conf = st.session_state.rk_type
            
            t_p = np.linspace(0, m_sim_time, int(m_sim_time * 20))
            sp_vector = np.ones(len(t_p)) * step_val
            
            pid_conf = PIDConfig(p_conf, n_conf, v_conf, rk_tf, type_conf, "ON", "OFF", sp_vector)
    ...
    with st.sidebar:
        ...
        st.file_uploader("📤 Load Config", type=["json"], key="uploader_key", on_change=on_upload_callback)
