...
SYSTEMS = {
    "PT1": {
        ...
        "b0": 4.6, "a0": 1.0, "a1": 21.0, "a2": 0.0,
        "tend": 120.0,
        "sp_max": 10.0, "sp_default": 5.0,
        "kp": 1.0, "tn": 15.0, "tv": 2.0,
    },
    "IT1": {
        "label": "IT1 — Integrating + Lag",
        "b0": 0.65, "a0": 0.0, "a1": 1.0, "a2": 17.0,
        "tend": 200.0,
        "sp_max": 50.0, "sp_default": 10.0,
        "kp": 0.5, "tn": 30.0, "tv": 5.0,
    },
    "PT2": {
        "label": "PT2 — Second-Order Lag (Overdamped)",
        "b0": 2.8, "a0": 1.0, "a1": 93.0, "a2": 972.0,
        "tend": 500.0,
        "sp_max": 10.0, "sp_default": 5.0,
        "kp": 0.5, "tn": 100.0, "tv": 15.0,
    },
    "PT2 Osc": {
        "label": "PT2 Osc — Oscillating Second-Order",
        "b0": 0.89, "a0": 1.0, "a1": 2.6, "a2": 27.04,
        "tend": 150.0,
        "sp_max": 10.0, "sp_default": 5.0,
        "kp": 1.0, "tn": 20.0, "tv": 3.0,
        "osc_T": 5.2, "osc_zeta": 0.25,
    },
}

DEFAULTS = {
    "system":              "PT1",
    # Plant parameters (overwritten by on_system_change)
    "b0":                  4.6,
    "a0":                  1.0,
    "a1":                  21.0,
    "a2":                  0.0,
...
def on_system_change():
    ...
    for k in ("b0", "a0", "a1", "a2", "tend"):
        st.session_state[k] = s[k]
    st.session_state["kp"] = s["kp"]
    st.session_state["tn"] = s["tn"]
    st.session_state["tv"] = s["tv"]
    if "osc_T" in s:
        st.session_state["osc_T"]    = s["osc_T"]
        st.session_state["osc_zeta"] = s["osc_zeta"]
    sp_max = float(s["sp_max"])
    st.session_state["sp_value"] = s["sp_default"]
    if st.session_state.get("sp2_value", 0.0) > sp_max:
...
def make_main_figure(t, pv_y, op, sp, SAT, dist_window=None):
    fig = make_subplots(
        rows=2, cols=1,
    ...
    if dist_window is not None:
        ...
        for row in (1, 2):
            fig.add_vrect(
                ...
                line=dict(color="rgba(255,80,80,0.6)", width=1, dash="dot"),
                row=row, col=1,
            )
        fig.add_annotation(
            x=(t0 + t1) / 2, y=1.0, yref="paper",
            text="⚡ Disturbance", showarrow=False,
            font=dict(color="#FF5050", size=11),
            bgcolor="rgba(255,80,80,0.15)",
            bordercolor="rgba(255,80,80,0.5)", borderwidth=1,
            xanchor="center",
        )
...
def make_components_figure(t, P_arr, I_arr, D_arr, ctrl_type):
    ...
    for idx, (name, arr, color) in enumerate(traces, 1):
        fig.add_trace(go.Scatter(
            ...
            fill="tozeroy", fillcolor=color.replace(")", ",0.08)").replace("rgb", "rgba") if color.startswith("rgb") else f"rgba({int(color[1:3],16)},{int(color[3:5],16)},{int(color[5:7],16)},0.08)",
        ), row=idx, col=1)
...
def render_sidebar():
    with st.sidebar:
        ...
        if sys_key == "PT2 Osc":
            st.slider("Gain", 0.0, 50.0, key="b0", step=0.01,
                      help="Static process gain")
            st.slider("Time Constant [s]", 0.1, 100.0, key="osc_T", step=0.1,
                      help="Natural time constant T = 1/ω₀")
            st.slider("Damping Factor ζ", 0.01, 0.99, key="osc_zeta", step=0.01,
                      help="Damping ratio — <1 underdamped, 1 = critically damped")
        elif sys_key == "IT1":
            st.slider("Gain", 0.0, 10.0, key="b0", step=0.05,
                      help="Static process gain")
            st.slider("Time Constant T₁ [s]", 0.0, 200.0, key="a2", step=1.0,
                      help="Lag time constant of the integrating system")
        elif sys_key == "PT2":
            st.slider("Gain", 0.0, 50.0, key="b0", step=0.1,
                      help="Static process gain")
            st.slider("Time Constant T₁ [s]", 0.0, 500.0, key="a1", step=1.0,
                      help="First time constant (coefficient a₁ = T₁+T₂)")
            st.slider("Time Constant T₂ [s]", 0.0, 10000.0, key="a2", step=1.0,
                      help="Second time constant (coefficient a₂ = T₁·T₂)")
        else:  # PT1
            st.slider("Gain", 0.0, 50.0, key="b0", step=0.1,
                      help="Static process gain")
            st.slider("Time Constant T₁ [s]", 0.0, 500.0, key="a1", step=1.0,
                      help="First-order time constant")
        ...
        ctrl = st.selectbox("Type", ["P", "PI", "PD", "PID"], key="ctrl_type")
        has_I = ctrl in ("PI", "PID")
        has_D = ctrl in ("PD", "PID")

        st.number_input("Kp — Proportional Gain", min_value=0.1, max_value=500.0,
                        step=0.1, format="%.1f", key="kp",
        ...
        st.download_button(
            "💾 Save all settings (JSON)", data=json.dumps(cfg_dict, indent=2),
            file_name="pid_config.json", mime="application/json",
            use_container_width=True,
        )
        st.file_uploader("📂 Load settings (JSON)", type="json",
...
def render_tab_simulation(t, pv, op, sp, e, P_arr, I_arr, D_arr, SAT, metrics, cfg):
    ...
    if cfg.dist_enable:
        ns = len(t) - 1
        dist_window = (t[int(0.40 * ns)], t[int(0.45 * ns)])
    st.plotly_chart(make_main_figure(t, pv_y, op, sp, SAT, dist_window),
                    use_container_width=True)
...
""")
    ...
    pa.caption("K=4.6, T₁=21 s")

    pb.markdown("**IT1** — Integrating + Lag")
    pb.latex(r"G(s) = \frac{K}{s\,(T_1 s + 1)}")
    pb.caption("K=0.65, T₁=17 s")

    pc.markdown("**PT2** — 2nd-Order Overdamped")
    pc.latex(r"G(s) = \frac{K}{(T_1 s+1)(T_2 s+1)}")
    pc.caption("K=2.8, T₁=12 s, T₂=81 s")

    pd.markdown("**PT2 Osc** — 2nd-Order Underdamped")
    pd.latex(r"G(s) = \frac{K \cdot \omega_0^2}{s^2 + 2\zeta\omega_0 s + \omega_0^2}")
    pd.caption("K=0.89, T=5.2 s, ζ=0.25")

# === 11. TAB: EXERCISES ===

def render_tab_exercises():
    st.subheader("Exercises — PID Controller")
    st.markdown("Use the sidebar controls to explore each scenario, then check your answers below.")
    st.divider()

    # --- Exercise 1 ---
    with st.expander("📝 Exercise 1 — P-Controller on a PT1 Plant", expanded=True):
        st.markdown("""
Select system **PT1** and controller type **P**. Use the defaults ($K_p = 1.0$, SP₁ = 5).

- **(a)** What steady-state error do you observe? Why does the P-controller not reach zero error?
- **(b)** Increase $K_p$ to 5.0. Does the error improve? What is the trade-off?
- **(c)** Switch to **PI** ($T_n = 15$). What happens to the steady-state error?
""")
        with st.expander("🔍 Solution (a)"):
            st.success("Steady-state error ≈ 0.89 (PV settles at ≈ 4.11 instead of 5.0).")
        with st.expander("💡 Explanation (a)"):
            st.markdown(r"""
For a PT1 plant with gain $K$ and a P-controller with gain $K_p$, the closed-loop
steady-state error is:

$$e_\infty = \frac{r}{1 + K_p \cdot K}$$

With $K = 4.6$ and $K_p = 1.0$: $e_\infty = 5 / (1 + 4.6) \approx 0.89$.
...
""")
        with st.expander("🔍 Solution (b)"):
            st.success("With Kp = 5.0: e_ss = 5/(1+23) ≈ 0.21 — much smaller, but the response overshoots.")
...
""")
    ...
    # --- Exercise 2 ---
    with st.expander("📝 Exercise 2 — Integrating System (IT1)"):
        st.markdown("""
Select system **IT1** and controller type **P**. Use the defaults ($K_p = 0.5$, SP₁ = 10).

- **(a)** Observe the response. Does the PV reach the setpoint?
- **(b)** Switch to **PI** with $T_n = 30$. What changes?
- **(c)** Enable Anti-Windup and disable it. Compare the overshoot after the initial transient.
""")
        with st.expander("🔍 Solution (a)"):
            st.success("With P-only control, the IT1 process can reach SP but may oscillate — the system itself contains an integrator.")
        with st.expander("💡 Explanation (a)"):
            st.markdown(r"""
The IT1 process already contains an integrator: $G(s) = \frac{0.65}{s(17s+1)}$.
...
""")
    ...
    # --- Exercise 3 ---
    with st.expander("📝 Exercise 3 — Derivative Action on an Oscillating Plant"):
        st.markdown("""
Select system **PT2 Osc** and controller type **PI** ($K_p = 1.0$, $T_n = 20$).

- **(a)** Enable the **Disturbance pulse** (at t=40 %). How quickly does the controller recover?
- **(b)** Switch to **PID** with $T_v = 3$. Does recovery improve?
- **(c)** Increase $T_v$ to 15. What happens?
""")
        with st.expander("🔍 Solution (b)"):
...
This makes PID especially effective for **disturbance rejection** in oscillating or slow plants.
""")
        with st.expander("🔍 Solution (c)"):
...
""")
    ...
    # --- Exercise 4 ---
    with st.expander("📝 Exercise 4 — Effect of Time Delay"):
        st.markdown(r"""
Select system **PT1** with controller type **PI** ($K_p = 1.0$, $T_n = 15$).
Keep all plant parameters at their defaults.

- **(a)** Set Time Delay to $0$ s. Note the settling time and overshoot.
- **(b)** Increase the delay to $5$ s, then $10$ s. How does the response change?
- **(c)** At $T_{delay} = 10$ s, try increasing $K_p$ to 5.0. What happens?

**Question:** Why does dead time make a controller with fixed gains increasingly unstable?
""")
        with st.expander("🔍 Solution (b)"):
            st.success("Larger dead time → more overshoot and slower settling. The controller acts on old information and overshoots before it can correct.")
        with st.expander("💡 Explanation (b)"):
            st.markdown(r"""
Dead time $T_{delay}$ means the controller only sees the effect of its action after a
delay. During this time the process continues to move in the same direction, causing
the PV to overshoot the setpoint.

In the frequency domain, dead time adds phase lag:
$$\angle e^{-j\omega T_{delay}} = -\omega T_{delay}$$

This reduces the phase margin of the loop, moving it closer to instability.
The larger the delay relative to the dominant time constant, the more the phase margin erodes.
""")
        with st.expander("🔍 Solution (c)"):
            st.success("Higher Kp with large dead time causes sustained oscillation or instability — the loop gain is too high for the available phase margin.")
        with st.expander("💡 Explanation (c)"):
            st.markdown(r"""
Increasing $K_p$ raises the crossover frequency $\omega_c$, where the additional
phase lag from dead time $\omega_c \cdot T_{delay}$ is even larger.
Once the phase margin reaches zero, the loop oscillates indefinitely.

**Rule of thumb:** When $T_{delay} / T_1 > 0.3$, standard PI tuning must be
de-tuned (lower $K_p$, higher $T_n$) to maintain adequate phase margin.
""")

    # --- Exercise 5 ---
    with st.expander("📝 Exercise 5 — Second Setpoint Step & Integral Windup"):
        st.markdown(r"""
Select system **PT2** and controller type **PI** ($K_p = 0.5$, $T_n = 100$).
Set SP₁ = 8, then enable the **second SP step** and set SP₂ = 2.
Set output limits to **Min = 0** and **Max = 100** (one-sided saturation).

- **(a)** Observe the response to both steps. Is the downward step as smooth as the upward one?
- **(b)** Set the output minimum back to $-100$ (symmetric limits) and repeat.
          Does this change the downward step response?
- **(c)** Now enable **Anti-Windup** and repeat step (a) with **Min = 0**.
          Does the recovery from the downward step improve?

**Question:** In which direction does integral windup occur for this scenario, and why?
""")
        with st.expander("🔍 Solution (a)"):
            st.success("The downward step response is slower and shows more overshoot — during the long period at OP_min=0, the integrator has wound up negatively.")
        with st.expander("💡 Explanation (a)"):
            st.markdown(r"""
After SP₁ is reached and the controller settles, the integral term holds a steady value.
When SP₂ drops sharply, the error becomes strongly negative, driving OP to its minimum
($0$ %). During the time the output is clamped at $0$ while the error is still negative,
the integrator continues to accumulate — **negative windup**.

When the PV eventually falls below SP₂, the large negative integral state means
OP stays near minimum for too long, causing undershoot and slow recovery.
""")
        with st.expander("💡 Explanation (c)"):
            st.markdown(r"""
Anti-Windup stops integration whenever the output is saturated **in the direction of the error**.
For the downward step with $e < 0$ and $u = u_{\min}$, the integrator is frozen —
preventing the negative accumulation.

This keeps the integral state near the correct steady-state value, so recovery
after the clamp is lifted is immediate rather than delayed.
""")
...
def main():
    ...
    st.caption("Interactive PID controller tuning simulation — adjust plant and controller parameters in the sidebar")
    st.divider()

    ss  = st.session_state
    tf  = max(ss["tv"] / 10.0, 1e-6)

    # Convert physical params to ODE coefficients for special systems
    if ss["system"] == "PT2 Osc":
        osc_T    = ss["osc_T"]
        osc_zeta = ss["osc_zeta"]
        sim_a0, sim_a1, sim_a2 = 1.0, 2.0 * osc_zeta * osc_T, osc_T ** 2
    elif ss["system"] == "IT1":
        sim_a0, sim_a1, sim_a2 = 0.0, 1.0, ss["a2"]
    else:
        sim_a0, sim_a1, sim_a2 = ss["a0"], ss["a1"], ss["a2"]

    cfg = SimCfg(
        b0=ss["b0"], a0=sim_a0, a1=sim_a1, a2=sim_a2,
        tdelay=ss["tdelay"],
        ...
        tend=ss["tend"], ts=ss["tend"] / 1000.0,
