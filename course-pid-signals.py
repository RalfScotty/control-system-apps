...
def on_upload_callback():
    """Triggered when a user uploads a JSON config file."""
...
def main():
    ...
    tab1, tab2 = st.tabs(["1. 🎛️ Signal Generator", "2. 📘 Theory & Information"])

    with tab1:
        # --- SIDEBAR: Inputs & File Operations ---
        with st.sidebar:
            st.header("⚙️ Signal Parameters")
            
            # Form (Dropdown) and Type
            ...
            keys_to_save = ['sig_form', 'sig_type', 'sig_freq', 'sig_amp', 'sig_offset', 'sig_phase', 'sig_samples', 'sig_noise']
            ...
            st.download_button(
                ...
                file_name=f"signal_config_{sig_form.lower()}.json", 
        ...
        with col_btn:
            st.download_button(
                ...
                data=csv,
                file_name=f'signal_{current_config.form.lower()}_data.csv',
                mime='text/csv',
                use_container_width=True
            )
